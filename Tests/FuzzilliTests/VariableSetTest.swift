...
struct VariableSetTests {
    @Test func testBasicVariableSetFeatures() {
        var s = VariableSet()
        #expect(s.isEmpty)

        #expect(!s.contains(v(0)))

        s.insert(v(42))
        #expect(s.contains(v(42)))

        s.insert(v(0))
        #expect(s.contains(v(0)))
        #expect(!s.contains(v(1)))
        s.insert(v(1))
        #expect(s.contains(v(1)))
        ...
        #expect(!s.contains(v(63)))
        #expect(!s.contains(v(64)))
        #expect(!s.contains(v(65)))
        ...
        #expect(s.contains(v(0)))
        #expect(s.contains(v(42)))
        #expect(!s.contains(v(62)))

        s.removeAll()
        #expect(s == VariableSet())
        #expect(s.isEmpty)
    }

    @Test func testVariableSetEquality() {
        ...
        #expect(s3 == VariableSet())
    }

    @Test func testVariableSetUnion() {
        var s1 = VariableSet([v(0), v(2), v(4), v(100)])
        var s2 = VariableSet([v(0), v(1), v(5), v(200)])

        let s3 = s1.union(s2)
        for i in 0...5 {
            #expect(i == 3 || s3.contains(v(i)))
        }
        #expect(s3.contains(v(100)))
    ...
    @Test func testVariableSetIntersection() {
        var s1 = VariableSet([v(0), v(2), v(4), v(100)])
    ...
    @Test func testVariableSetIsSubset() {
        ...
        #expect(!s4.isSubset(of: s5))
        let s6 = VariableSet([v(0)])
        #expect(s6.isSubset(of: s4))
        #expect(!s6.isSubset(of: s5))
        #expect(!s4.isSubset(of: s6))
        #expect(!s5.isSubset(of: s6))
    }

    @Test func testVariableSetIsDisjoint() {
        let s1 = VariableSet([v(0), v(2), v(4), v(100)])
        let s2 = VariableSet([v(0), v(1)])
        #expect(!s1.isDisjoint(with: s2))
        #expect(!s2.isDisjoint(with: s1))
        #expect(!s1.isDisjoint(with: s1))

        let s3 = VariableSet([v(0), v(100), v(200)])
        let s4 = VariableSet([v(1)])
        #expect(s3.isDisjoint(with: s4))
        #expect(s4.isDisjoint(with: s3))
        #expect(!s3.isDisjoint(with: s3))

        let s5 = VariableSet([v(0), v(64)])
        #expect(s5.isDisjoint(with: [v(1)]))
        #expect(!s5.isDisjoint(with: [v(0)]))
    }

    @Test func testVariableSetSubtraction() {
    ...
    @Test func testVariableSetIteration() {
